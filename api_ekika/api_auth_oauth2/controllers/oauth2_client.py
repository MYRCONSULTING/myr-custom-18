...
class ApiOAuth2ClientController(http.Controller):
    ...
    def get_client_params(self):
        ...
        return client_id, client_user_identity

    @http.route(route='/<api>/oauth2/provider/authorize', methods=['GET'],
                type='http', auth='none', save_session=False)
    def api_oauth2_provider_authorize(self, api, **kwargs):
        """
        ...
        """
        client_id, client_user_identity = self.get_client_params()

        api_rec = request.env['easy.api'].with_user(SUPERUSER_ID)\
                  .search([('base_endpoint','=',api)])
    ...
    @http.route(route='/<api>/oauth2/callback', methods=['GET'], type='http',
                auth='none', save_session=False, csrf=False)
    def oauth2_client_callback(self, api, **kwargs):
        """
        ...
        api_rec = request.env['easy.api'].sudo().search([('base_endpoint','=',api),
    ...
    @http.route(route='/<api>/oauth2/token', methods=['POST'], type='http',
                auth='none', save_session=False, csrf=False)
    def oauth2_client_oauth2_token(self, api, **kwargs):
        """
    ...
    @http.route(route='/<api>/oauth2/revoke', methods=['GET'], type='http',
                auth='none', save_session=False, csrf=False)
    def oauth2_client_revoke_token(self, api, **kwargs):
        """
