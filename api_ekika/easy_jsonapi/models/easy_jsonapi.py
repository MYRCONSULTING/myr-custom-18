...
class JsonAPIQuery():
    ...
    def __init__(self, request):
        ...
        the_api_record = request.easyapi.get('record')
        if the_api_record and the_api_record.jsonapi_use_body_for_get is True:
            self.params = self.request.get_json_data()
        else:
...
class JsonAPIResponse(Response):
    ...
    def render(self):
        ...
        return Markup(json.dumps(result, ensure_ascii=False, default=json_default))
...
class EasyJsonApi(models.TransientModel):
    ...
    def serve_post(cls, query, response):
        ...
        if model_field == 'execute':
            ...
            try:
                ...
                json.dumps(values, ensure_ascii=False, default=json_default)
            except Exception as exc:
    ...
    def serve(cls):
        ...
        default_meta = {
            "version": "1.1",
            "author-info": {
                'author': request.easyapi['record'].meta_author_name,
                'author-email': request.easyapi['record'].meta_author_email,
            },
            "documentation-link": request.easyapi['record'].meta_documentation_link,
        }
