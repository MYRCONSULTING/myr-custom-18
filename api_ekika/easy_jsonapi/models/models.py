        ...
        if selflink:
            return {
                "links": {
                    ...
                    "describedby": request.easyapi['record'].meta_documentation_link
                },
...
class Base(models.AbstractModel):
    ...
    def generate_create_field_values(self, data):
        ...
        if 'relationships' in data['data']:
            relational_fields = [k  for key,val in data.items()
                                 if isinstance(val,dict) and 'relationships' in val for k,v in val['relationships'].items()]
    ...
    def generate_write_fields_values(self, data, relationships=None, model_field=None, method=None):
        if relationships:
            ...
            if 'relationships' in data['data']:
                relational_fields = [k  for key,val in data.items()
                                     if isinstance(val,dict) and 'relationships' in val
                                     for k,v in val['relationships'].items()]
                # relational_fields = []
                # for key,val in data.items():
                #     if isinstance(val, dict) :
                #         for k,v in val['relationships'].items():
                #             relational_fields.append(k)
