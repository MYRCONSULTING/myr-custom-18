...
class ApiLogControllers(HttpController):
    ...
    def log(self, api_id, tail_length=-25):
        ...
        tail_length = abs(tail_length)
        tail = DEFAULT_LOG
        try:
            # Use tail command to get the last tail_length lines
            result = subprocess.run(['tail', f'-n{tail_length}', log_file], capture_output=True, text=True)
            if result.returncode == 0:
                tail = result.stdout
            else:
                tail = f"Error: {result.stderr}"
        except FileNotFoundError:
            return f"""
                <h1>Log-File: "{log_file}" Not Found.</h1>
                """
            ...
            return f"""
                <h1>Memory Error: Unable to read log file.</h1>
                """
