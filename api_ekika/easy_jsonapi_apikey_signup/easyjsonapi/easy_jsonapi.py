...
class SignUpEasyJsonApiKey(models.TransientModel):
    ...
    def serve_public_jsonapi_key(cls):
        ...
        if not (request.env.user and request.env.user.is_public):
            ...
            for url, endpoint in super()._generate_routing_rules(modules, converters):
                if url == '/_jsonapi_key_dynamic_api_route_public':
                    for endpoint_url in all_endpoints:
                        yield endpoint_url, endpoint
                else:
                    yield url, endpoint


class JsonAPIKeyPublic(JsonAPIController):
    @jsonapiRoute(['/_jsonapi_key_dynamic_api_route_public'], auth='public', csrf=False, type='api', cors='api', methods=['POST'])
